...
import type { ContentDashboardData, DashboardFilters, FilterOptions } from "@/lib/types";
import { AppShell } from "@/components/AppShell";
import { FilterBar } from "@/components/dashboard/FilterBar";
import { BrandSentimentMap } from "./BrandSentimentMap";
import { EngRateByMarketChart } from "./EngRateByMarketChart";
import { ImpressionsByMarketChart } from "./ImpressionsByMarketChart";
...
export function ContentDashboard() {
  ...
  const fetchData = useCallback(async (currentFilters: DashboardFilters) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/content?${buildQueryString(currentFilters)}`);
      const json = await res.json();
      if (!json.data) {
        setData(null);
        setError(
          "No content data found for the selected filters. Try widening your date range or filters.",
        );
      } else {
        setData(json.data);
      }
      ...
      <header className="min-w-0 shrink-0 border-b border-brand/10 bg-surface px-6 py-4">
        <div className="mb-3">
          ...
          <p className="text-xs text-muted">
            Creator performance, impressions, engagement, and audience sentiment across
            your markets.
          </p>
        </div>
        <FilterBar
        ...
        {loading ? (
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 items-stretch gap-5">
              <ImpressionsByMarketChart data={data.impressionsByMarket} />
              <EngRateByMarketChart data={data.engRateByMarket} />
            </div>

            <BrandSentimentMap sentimentByBrand={data.sentimentByBrand} />
